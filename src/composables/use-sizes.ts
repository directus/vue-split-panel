...
export const useSizes = (size: Ref<number>, options: UseSizesOptions) => {
	...
	const componentSize = computed(() =>
		toValue(options.orientation) === "horizontal" ? componentWidth.value : componentHeight.value,
	);

	const { width: dividerWidth, height: dividerHeight } = useElementSize(options.dividerEl);
	const dividerSize = computed(() =>
		toValue(options.orientation) === "horizontal" ? dividerWidth.value : dividerHeight.value,
	);
	...
	const snapPixels = computed(() => {
		...
		return toValue(options.snapPoints).map((snapPercentage) =>
			percentageToPixels(componentSize.value, snapPercentage),
		);
	});
